...
# # ST3_Budget (Acu) — Bronze to Silver Transformation
# 
# **Source table:** `bronze.bronze_acu_Budget_Pneumatic`  
# **Target table:** `silver.silver_acu_ST3_Budget`
# 
# This notebook replicates, step by step, the Power Query M logic used to build the **ST3_Budget(Acu)** table from the **Budget - Pneumatic** Acumatica OData source.
...
# ## 2. Load Source Table
# Equivalent to the `Source` + `Navigation 1` steps — reads the Budget - Pneumatic data already landed in bronze.

# CELL ********************

df = spark.table("bronze.bronze_acu_Budget_Pneumatic")
display(df.limit(10))
...
# - `BudgetMonth` → `Month`
# - `ST3_Formulaa954ce773e844415b80dfcc9fd7d8630` → `BU`
...
df = (
    df.withColumnRenamed("CustomerCode", "Customer_ID")
      ...
      .withColumnRenamed("ST3_Formulaa954ce773e844415b80dfcc9fd7d8630", "BU")
)
...
df.write.mode("overwrite").format("delta").saveAsTable("silver.silver_acu_ST3_Budget")
print("Write complete: silver.silver_acu_ST3_Budget")
