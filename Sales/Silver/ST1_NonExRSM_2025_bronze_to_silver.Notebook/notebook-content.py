...
# # ST1_NonExRSM 2025 — Bronze to Silver Transformation
# 
# **Source table:** `bronze.bronze_acu_NewBudget_RSM_Solid`  
# **Target table:** `silver.silver_acu_ST1_NonExRSM_2025`
# 
# This notebook replicates, step by step, the Power Query M logic used to build the **ST1_NonExRSM 2025** table from the **NewBudget_RSM_Solid** Acumatica OData source. This mirrors the ST3_NonExRSM 2025 table structurally, just against the Solid business unit's RSM source data.
...
# ## 2. Load Source Table
# Equivalent to the `Source` + `Navigation` steps — reads the NewBudget_RSM_Solid data already landed in bronze.

# CELL ********************

df = spark.table("bronze.bronze_acu_NewBudget_RSM_Solid")
df.printSchema()
display(df.limit(10))
...
df.write.mode("overwrite").format("delta").saveAsTable("silver.silver_acu_ST1_NonExRSM_2025")
print("Write complete: silver.silver_acu_ST1_NonExRSM_2025")
